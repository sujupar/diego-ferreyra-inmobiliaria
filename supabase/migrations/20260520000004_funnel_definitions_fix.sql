-- Métricas — redefinir vw_funnel_daily y get_meta_funnel_by_campaign
--
-- BUGS REPORTADOS POR EL USUARIO:
--   1. Las métricas en /metrics no coinciden con la realidad del CRM.
--      Aparecen "exageradas" en rangos multi-día.
--   2. El usuario quiere "visitas a la página" en lugar de "clics" en la
--      sección Meta Ads.
--
-- CAUSA RAÍZ DEL BUG #1 (embudo CRM):
--   La vista original contaba como "appraisal_requests" TODO deal cuyo
--   `origin IS DISTINCT FROM 'clase_gratuita'`. Eso incluía:
--     - origin='embudo'    (correcto: registros directos vía GHL form)
--     - origin='referido'  (cargados manualmente, NO son solicitudes)
--     - origin='historico' (datos heredados de antes del sistema)
--     - origin='comprador' (otro pipeline distinto)
--     - origin=NULL        (deals creados desde la UI sin marcar origen)
--   Los últimos cuatro inflaban el conteo en 3-5x según la auditoría.
--
--   Solución: filtrar a `origin = 'embudo'` para solicitudes de tasación.
--   Y restringir las métricas de eventos (agendadas, visitas, entregadas,
--   captadas, perdidas) a deals con origin del embudo de marketing
--   (`'embudo'` o `'clase_gratuita'`), para no contar referidos/históricos
--   que no son parte del funnel medible.
--
-- CAMBIO PARA #2:
--   La RPC get_meta_funnel_by_campaign ahora devuelve landing_page_views
--   (en lugar de clicks). CTR se recalcula como LPV/impressions.
...
-- ============================================================================
-- 2) get_funnel_metrics RPC — re-creada para usar la vista nueva.
--    (La definición no cambia, solo se re-crea por defensa después del CASCADE.)
-- ============================================================================
...
--    Nota: la respuesta agrega ambos clicks (para compat) y landing_page_views.
--    El dashboard usará landing_page_views; CTR ahora se calcula sobre LPV.
-- ============================================================================
