...
- **Las distancias salen del MAPA, nunca de la web:** dos búsquedas web sobre la misma dirección dieron 300 m y 800 m al mismo subte. `zona-mapa.ts` consulta Overpass (OpenStreetMap): estaciones con su línea (vía la relación de ruta del `stop_area`), plazas, colegios (sin los de adultos), hospitales y **colectivos que pasan a menos de 400 m** (la web decía "líneas que circulan por Almagro", que no es lo mismo). La web solo aporta carácter del barrio, comercios e hitos, y sus citas/links se limpian (`limpiar-web.ts`).
