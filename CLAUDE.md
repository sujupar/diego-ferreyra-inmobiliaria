...
- **Pendientes que dejó la revisión (NO hechos):** (a) las métricas de VIDEO de la A y la B se mezclan: las dos usan `trackKey="hero-tasacion"` siendo videos distintos; (b) enviar el formulario desde adentro del visor crea un lead real con CAPI (el submit no mira `hm_preview`); (c) las RPC traen todas las páginas sin `ORDER BY`: con rangos largos podrían tocar el tope de 1000 filas de PostgREST; (d) `pageHeatGrid` viaja en `/api/funnels/metrics` y nadie lo consume.
