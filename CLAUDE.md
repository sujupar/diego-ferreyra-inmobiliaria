...
- **Reglas vigentes (decisiones del dueño):**
  ...
  - **Aviso de proceso a medias** en la ficha (`faltantesDelProceso`): dice QUÉ falta (nombre real, teléfono, asesor). **No aplica a `request`/`clase_gratuita`/`lost`/`comprador`** — una solicitud del embudo todavía no tiene asesor por diseño; sin esa excepción el aviso salía en cada lead nuevo. "El nombre es la dirección" exige un número en el nombre ("Ana" en "Anatole France 200" no es una dirección).
