...
- **Flujo un-paso + continuación del lado del cliente:** `POST /api/social/carousels` genera el guion e inserta slides `pending`. El front pollea `GET /api/social/carousels/[id]`, y **cada GET procesa UN slide pendiente** (`processNextSlide`) — así entra en `maxDuration=60` sin cron ni background function. `PATCH .../slides/[n]` edita copy (re-render gratis) o `regenerate` (1 gpt-image-2). `POST .../export` = ZIP (fflate). UI en `app/(dashboard)/redes-sociales/`.
