...
- **Detection / verificación post-deploy:** Tras commitear+pushear (re-deploya y RE-REGISTRA el cron), confirmar en Netlify → Functions que las 4 `scheduled-*-report` aparecen con schedule, y que la siguiente corrida deja un registro `sent`/`failed`/`skipped` en `email_report_log`. Antes de declarar "los reportes andan", verificar que existe un registro a la hora del cron, no solo envíos manuales.
