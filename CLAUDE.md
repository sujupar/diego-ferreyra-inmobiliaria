...
- **TRAMPA 2 — dentro de comillas de PostgREST hay que DUPLICAR la barra invertida.** PostgREST usa `\` como su propio escape y se come una: `"2\*D"` llega a Postgres como `2*D` (otra vez 17 fichas); `"2\\*D"` llega como `2\*D` y devuelve 1. Y las comillas no son opcionales: una coma cruda rompe el árbol lógico (`PGRST100`). Son dos capas de escapado distintas: `patronRegex` produce barras simples, `valorPostgrest` las duplica. **No mezclarlas.**
- **TRAMPA 3 — macOS entrega el texto DESCOMPUESTO (NFD).** La "ó" copiada del Finder, de un PDF o de la terminal son DOS caracteres: `o` + U+0301. Sin `normalize('NFC')` el patrón exige esa tilde suelta literal y la búsqueda devuelve **cero fichas, sin error** — el asesor concluye que la propiedad no está cargada. Es el mismo U+0301 que rompe Turbopack por el nombre de esta carpeta (abajo), o sea el entorno real de trabajo, no un caso exótico.
- **TRAMPA 4 — el tope del TEXTO no acota la DIRECCIÓN.** Cada vocal se vuelve una clase de 6 caracteres que codificada pesa ~35, y cada palabra repite la condición para todas las columnas: la misma cantidad de texto produce direcciones de 1,4 KB a 15 KB. Medido: 33 palabras de una letra (65 caracteres, dentro del tope de 80) → dirección de 10,9 KB → `UND_ERR_HEADERS_OVERFLOW` → **500**. Por eso el tope real (`MAX_CARACTERES_CONSULTA`) es sobre lo que pesa la consulta, no sobre el texto.
...
- **OJO con `getPropertiesListPage`:** corre DOS consultas paginadas en paralelo (la vista y `properties` para dos banderas) y las cruza por id asumiendo el mismo conjunto. Por eso los filtros viven en UNA sola función, `aplicarFiltrosListado`, que alimenta a las dos. **Agregar un filtro a una sola rompe el cruce en silencio.** Y `description` no se puede buscar sin tocar la vista: no está en ella.
- **Un control con espera necesita las TRES reglas juntas** (`lib/filters/borrador-filtro.ts`): el campo no se lee de la dirección (iría a los saltos), pero tampoco la ignora ("Limpiar todo" y el botón atrás), y el valor que vuelve después de escribir es el ECO propio y no un cambio de afuera. Además, **la espera pendiente hay que cancelarla cuando el filtro cambia desde afuera**: sin eso, escribir y tocar "Limpiar todo" hacía que la limpieza se deshiciera sola unos milisegundos después.
- **Verificación:** `scripts/`-style probe contra la API real (13 chequeos). Nada de esto se dedujo — todo se comprobó pegándole a Supabase.

### Correr las pruebas en esta Mac: vitest tarda 13 minutos o directamente no arranca

- **Symptom 1:** `npx vitest run` sobre un archivo suelto se queda 7+ minutos a 0% de CPU. **Causa:** la config raíz rastrea el proyecto ENTERO (videos, capturas, `node_modules` anidados bajo `video/`) sobre una carpeta que vive en iCloud. **Fix:** una config acotada con `include` de las carpetas que se están tocando. Con eso los tests de node corren en ~0,5 s.
- **Symptom 2:** los tests de COMPONENTE (`.test.tsx`, happy-dom) fallan con `[vitest-pool]: Failed to start ... worker` a los **exactamente 60 s**. **Causa:** `happy-dom` son 581 archivos ESM y en frío tardan más de un minuto en cargar sobre este disco; `START_TIMEOUT = 6e4` está **hardcodeado** en vitest, no se puede subir por configuración. **No es tu código:** los `FilterBar.test.tsx` y `DateRangeFilter.test.tsx` que ya existían fallan igual. **Fix:** correrlo una vez y esperar (una corrida tardó 806 s, de los cuales 805 fueron el import); una vez que el caché de archivos del sistema está tibio, las corridas siguientes tardan ~55 s.
- **Symptom 3:** `userEvent` con `vi.useFakeTimers()` cuelga CUALQUIER interacción —hasta un clic— hasta el límite de 5 s. **Fix:** no usar temporizadores simulados; los controles con espera aceptan `esperaMs` por prop, así que se pasa un valor larguísimo para probar que NO pasa nada y uno cortísimo con `waitFor` para esperar a que pase.
- **`npx tsc --noEmit` sobre el `tsconfig.json` raíz también se cuelga** (mismo motivo). Usar un tsconfig acotado que extienda el raíz — la práctica que ya insinúan los `tsconfig.*.tsbuildinfo` del repo.
