...
### Toda tasación y toda captación pertenece a un proceso del CRM (2026-09-17)

- **Symptom:** las tasaciones y captaciones cargadas A MANO (referidos, históricos, gente que llamó) no aparecían en el CRM del asesor, saltaban de etapa, mandaban un email falso de "Tasación agendada", y captar desde la ficha de la tasación dejaba el proceso en "Tasación Entregada" para siempre. Hipólito Yrigoyen 1550 se captó dos veces.
- **Root cause:** la tasación manual creaba el proceso desde el NAVEGADOR con tres `fetch` encadenados: contacto = la dirección, sin teléfono, sin asesor (el asesor solo ve lo asignado a él), origen forzado. Y el vínculo captación↔proceso también lo hacía el navegador, solo si se entraba con `?dealId=`. Spec con la evidencia: `docs/superpowers/specs/2026-09-17-procesos-manuales-en-crm-design.md`.
- **Reglas vigentes (decisiones del dueño):**
  - **Primero de quién es.** `/appraisal/new` y `/properties/new` sin proceso muestran `ElegirCliente` (`components/deals/ElegirCliente.tsx`): buscar un proceso (`GET /api/deals/buscar`, con el mismo alcance por rol que el CRM) o cargar el cliente → `POST /api/deals/manual`, que valida TODO en el servidor (`validarClienteNuevo`, `lib/deals/proceso-manual.ts`). Un asesor solo crea procesos para sí mismo.
  - **Sin orígenes nuevos:** embudo / referido / histórico (lo que aceptan los CHECK de `deals.origin` y `contacts.origin`). "Tasación" NO es un origen: la base lo rechaza con 23514.
  - **Sin saltear etapas:** la tasación manual nace en "Coordinada" con su fecha, pasa por el formulario de visita y sigue con los botones de siempre. `linkAppraisalToDeal` ya NO cambia la etapa: crear la tasación ≠ entregarla.
  - **El vínculo captación↔proceso lo hace el servidor** en `POST /api/properties` (`deal_id` explícito, o resuelto por `appraisal_id`; con dos procesos no adivina). Si el proceso ya tiene una ficha activa → **409 con `propertyId`** y la pantalla abre esa ficha. Lo cargado en la visita (expensas, portales, landing) se hereda leyéndolo del proceso EN EL SERVIDOR; lo que manda la pantalla gana. Sin proceso NO se frena el alta (las tasaciones viejas sin proceso tienen que poder captarse) pero responde `avisoProceso`.
  - **Permiso para mover el proceso:** `pipeline.advance` + `canAccessDeal` (un asesor, solo los suyos). Al pasar el vínculo del navegador (`/advance`, que pedía ese permiso) a esta ruta (que solo pedía estar logueado) se abrió un agujero —un abogado podía pasar cualquier proceso a "Captada" con un `deal_id`— que se cerró antes del merge. **Al mover una escritura de una ruta a otra, mover también su control de permisos.**
  - **Reasignar asesor** (`PUT /api/deals/[id]` con `assigned_to`): solo `pipeline.view_all` y solo hacia un asesor/dueño activo. `puedeReasignarAsesor` es la misma regla en la ruta y en la pantalla.
  - **Aviso de proceso a medias** en la ficha (`faltantesDelProceso`): dice QUÉ falta (nombre real, teléfono, email, asesor). **No aplica a `request`/`clase_gratuita`/`lost`/`comprador`** — una solicitud del embudo todavía no tiene asesor por diseño; sin esa excepción el aviso salía en cada lead nuevo. "El nombre es la dirección" exige un número en el nombre ("Ana" en "Anatole France 200" no es una dirección).
  - **Datos de la visita después de la visita:** botón en Visita Realizada / Entregada / Seguimiento / Captada que abre el formulario en modo `editar` — NO manda `complete`, no mueve la etapa. "Finalizar Visita" ahora mira la respuesta del servidor.
- **Datos del cliente obligatorios para CREAR y para AVANZAR (2026-09-18, PR #16).** Nombre real, teléfono (10 dígitos), **email (100% obligatorio, decisión del dueño)** y asesor.
  - **Al crear:** tasación/captación manual (`/api/deals/manual`) y **"Coordinar tasación"** (`POST /api/deals`, que antes solo pedía nombre y dirección) validan con `validarDatosCliente`. Un asesor coordina a SU nombre.
  - **Al avanzar:** la barrera está en el SERVIDOR, en las tres rutas que mueven `deals.stage` — `/api/deals/[id]/advance`, `/visit-data` al finalizar y `POST /api/properties` al captar. Si falta algo responden `422 {code:'DATOS_DEL_CLIENTE', faltan, dealId}` y NO mueven nada (Finalizar Visita guarda igual lo cargado; Captar no crea la ficha). Regla única: `camposFaltantes` + `exigeDatosParaMover` en `lib/deals/proceso-manual.ts`. **No exigen datos:** descartar (`lost`), no se realizó (`not_visited`), y quedarse en la misma etapa (otro seguimiento dentro de Seguimiento; también re-captar un proceso que ya está en `captured`). Vincular la tasación recién creada no es mover la etapa.
  - **La pantalla:** ventana `components/deals/CompletarDatosCliente.tsx` ("Completá los datos del cliente para avanzar") que guarda con `PUT /api/deals/[id]/cliente` y repite sola la acción pedida. **Trampa:** el reintento NO vuelve a chequear con los datos de pantalla (en ese instante todavía son los viejos y la ventana se abre en bucle): va directo y decide el servidor (`yaCompletado`). **Otra:** el chequeo del seguimiento va ANTES de crear la tarea, o completar y reintentar duplicaba la tarea. Y `onClick={handleFollowupSubmit}` pasaría el evento del clic como `yaCompletado`: usar `() => handleFollowupSubmit()`.
  - **`/cliente` no pisa a otra persona:** si el proceso ya tiene contacto, se corrige ese; si no, se busca por email/teléfono y del contacto encontrado se completan SOLO los huecos o lo inválido. Un teléfono mal tipeado no puede renombrar a otro contacto.
  - **Detection:** `SELECT count(*) FROM deals d LEFT JOIN contacts c ON c.id=d.contact_id WHERE d.stage IN ('scheduled','not_visited','visited','appraisal_sent','followup') AND (c.email IS NULL OR c.phone IS NULL OR d.assigned_to IS NULL);` — el 2026-09-18 daba 190 de 200: se completan cuando alguien los mueve, no en masa.
- **Reparación de datos:** `scripts/reparar-procesos-manuales.ts` (modo informe por defecto, `--commit` para escribir; relee cada fila antes de escribir y verifica después). Lógica pura y testeada en `lib/deals/reparacion-procesos.ts`: vincula captaciones con su proceso, conserva la ficha MÁS NUEVA ante duplicados (la otra → `descartada` con motivo, nunca se borra), y LISTA lo que no puede arreglar solo (procesos a medias, tasaciones sin proceso, direcciones con dos procesos abiertos). Ante la duda no toca: lo reporta como conflicto. Informe del 2026-09-17: 4 vínculos, 1 descarte (Hipólito 14/9), 179 procesos a medias (148 solo sin asesor, mayormente importados de GHL/CSV), 4 tasaciones sin proceso.
- **Detection:** `SELECT stage, count(*) FROM deals WHERE assigned_to IS NULL AND stage NOT IN ('request','clase_gratuita','lost','comprador') GROUP BY stage;` — un proceso abierto sin asesor no le aparece a nadie. Y una propiedad con `appraisal_id` cuyo proceso tiene `property_id IS NULL` es una captación que no movió su proceso.
...
- **Botón "Responder al interesado" (`consulta_portal_v2`, APROBADA como UTILITY el 2026-08-28, activa en Netlify desde fines de agosto):** cuerpo CALCADO del aprobado —cambiarlo sería volver a jugarse la categoría; la gemela `nueva_consulta_portal` cayó en MARKETING y Meta la RETIENE— más un botón URL a `inmodf.com.ar/r/{{1}}`. Meta la aceptó como UTILITY con el botón apuntando a nuestro dominio. **El link SIGUE en el cuerpo, no es redundancia:** en WhatsApp de computadora los botones de plantilla no abren nada (verificado por el dueño el 2026-08-03, es lo que motivó `recorrido_acceso_v4`). Botón para el celular, link para la compu.
- **Plantilla y botón se deciden JUNTOS, en `lib/integrations/portal-inquiries/plantilla-del-aviso.ts`** (`elegirPlantillaDelAviso` + `PLANTILLA_SIN_BOTON`). Meta rechaza el envío ENTERO en los dos sentidos: botón mandado a una plantilla que no lo declara, y plantilla con botón mandada sin el dato del botón. Con link al chat del interesado sale `consulta_portal_v2` con botón; sin link, su gemela `consulta_portal_util` (mismo texto carácter por carácter y mismos 10 datos, verificado en la API de Meta el 2026-09-17). Una plantilla con botón NUEVA hay que darla de alta ahí con su gemela ANTES de configurarla en Netlify, o todos los avisos rebotan.
- **INCIDENTE 2026-09-17 — avisos rechazados en silencio 12 días:**
  - **Síntoma:** llegaba el mail del portal pero no el WhatsApp, ni a Diego ni a la copia. Caso que lo destapó: la #407 (Roque Pérez 3050).
  - **Causa:** con `consulta_portal_v2` en Netlify, el código mandaba la plantilla con botón aunque no hubiera link corto. El link corto solo existe si el interesado dejó un teléfono VÁLIDO, y muchas veces no: MercadoLibre lo oculta, en Argenprop muchos dejan solo el email, y otros lo escriben mal (`541103305864`, `44652269`). Meta respondía `(#131008) Required parameter is missing` y rechazaba el mensaje entero. Entre el 5 y el 17/9: 12 consultas, 7 de personas reales (las otras 5 eran publicidad de Argenprop). La alarma de sequía tenía el mismo defecto: con la v2 configurada, la alarma que avisa que se cortaron las consultas tampoco habría salido.
  - **Fix:** `elegirPlantillaDelAviso`, usada por `notify.ts` y por la alarma de sequía. Las 7 reales se reenviaron por el mismo `notifyInquiry` con la configuración de producción: 14/14 aceptados por Meta.
  - **Detección:** nada avisaba — el rechazo queda anotado en `portal_inquiry_notifications` y el cron responde "ok". Ante "llegó el mail pero no el WhatsApp", lo primero es `SELECT status, error_message FROM portal_inquiry_notifications WHERE inquiry_id = …`; para ver el alcance, `… WHERE status='failed' AND created_at > now() - interval '14 days'`.
  - **Pendiente de antes, no de este fix:** `cuerpos-aprobados.ts` todavía no tiene `consulta_portal_v2` (falta correr `scripts/sincronizar-cuerpos-plantillas.ts`), así que el Inbox muestra los datos sueltos en vez del texto real de los avisos que salen con botón.
- **Correr `notify.ts` desde un script con `tsx`:** choca con dos cosas que en la app resuelve Next — el paquete `server-only` no está instalado, y la metadata de `libphonenumber-js/max` llega rota bajo `tsx` ("Cannot read properties of undefined (reading 'hasOwnProperty')"). Se esquivan SIN tocar `node_modules` con una precarga CJS pasada como `node --require <precarga>.cjs --import tsx …`, que (1) hace `require('libphonenumber-js/max')` antes de que `tsx` registre sus hooks, así queda sana en el caché, y (2) redirige `server-only` a un módulo vacío vía `Module._resolveFilename`.
...
### Fotos incrustadas (base64) en `properties.photos`: el 413 de ML y el "Multimedia.Url" de Argenprop (2026-09-14)

- **Symptom:** publicar Av. Hipólito Yrigoyen 1550 en vivo dio `MercadoLibre rechazó el aviso (error 413)` y Argenprop `400 PUB001 Multimedia.Url: Valor inválido: data:image/png;base64,…` (un JSON de 4 MB en pantalla).
- **Root cause:** el asistente de tasación guarda las fotos como `data:image/...;base64` (`FileReader.readAsDataURL`) en `appraisals.property_images`, y `/properties/new?dealId=` las heredaba **tal cual** a `properties.photos`. Una captura de Street View de 4,4 MB viajaba entera en el body del aviso de ML (→ 413) y Argenprop valida la URL (→ inválida). Ningún consumidor filtraba el esquema.
- **Decisión del dueño (mismo día, PR #10): el alta desde una tasación YA NO hereda las fotos de la tasación.** Son material del informe (Street View, mapa), no del aviso; las fotos del aviso se suben a mano en Multimedia y recién ahí la propiedad pasa a captada. `properties/new` no lee `property_images` (hay un test que lo fija). Lo de abajo queda como red de seguridad.
- **Fix:** (1) `POST /api/properties` saca las `data:` del INSERT, las sube a Storage (`lib/properties/materializar-fotos.ts`, path `properties/{id}/photos/{uuid}.{ext}`) y guarda las URLs en el mismo orden, ANTES de `checkAndAdvanceProperty`. (2) Los dos mappers (`pictures` de ML, `Multimedia` de AP) y `validateCommon` pasan por `fotosPublicables()` (`lib/portals/fotos-publicables.ts`): solo `https://`, y el descarte se explica con la posición de la foto. (3) `scripts/reparar-fotos-incrustadas.ts [--commit]` para el dato viejo (corrido en producción el 2026-09-14: 1 de 28 propiedades activas).
- **Errores legibles:** `lib/portals/errores-legibles.ts` traduce (413 → "el aviso es demasiado pesado… imagen incrustada… Multimedia"; Argenprop por campo: `Multimedia.Url`, `Titulo`, `Precio`, `Localizacion.*`…). Argenprop ahora lanza `PortalAdapterError(legible, …, original=crudo)` como ML y su ruta usa `mensajeYDetalle`, así `soloElMensaje` sirve para los dos. El detalle crudo se recorta a 1500 caracteres (`recortarDetalle`) antes de guardarse. Los paneles de gestión y las tarjetas de Difusión muestran solo el mensaje, `break-words line-clamp-4`.
- **Detection:** `select count(*) from properties where status<>'descartada' and exists (select 1 from unnest(photos) p where p like 'data:%')` tiene que dar 0. Si un `last_error` mide más de 2000 caracteres, alguien volvió a guardar el cuerpo crudo sin recortar.
- **Trampa de TypeScript:** `esFotoIncrustada` NO es un type guard (`valor is string`) a propósito: con guard, en el `false` de un ternario `f` quedaba `never` y `f.slice` no compilaba.

### Datos de difusión en la visita → portales y landing en autopilot (2026-09-14)

- **Dónde se cargan:** formulario de visita (`components/pipeline/VisitDataForm.tsx`), pestaña Venta, Secciones 08 "Datos para portales" y 09 "Datos para la landing" (`VisitDifusionSections.tsx`). Viven en `deals.visit_data` bajo `portales` (`{expensas, ml, ap}`) y `landing` (`{q1..q4}`), saneadas en el servidor (`lib/supabase/visit-data-sanear.ts`). Los campos de ML salen del schema REAL de la categoría de venta (`GET /api/deals/[id]/campos-portales?tipo=`), menos los que ya se derivan de la visita y los administrativos (`ATRIBUTOS_ML_ADMINISTRATIVOS`); los booleanos van como checklist (marcado = "Sí", sin marcar = no se manda). Argenprop pide solo Subtipo y Estado. Las expensas se piden UNA vez (campo propio → `properties.expensas`).
- **Herencia al captar:** `/properties/new?dealId=` manda `expensas`, `portal_data` y `landing_answers` (`armarDatosDifusionDesdeVisita` en `lib/portals/datos-visita.ts`: derivados de la visita + respuestas; lo contestado manda). Columnas `properties.portal_data` y `landing_answers` (jsonb, default `{}`, migración `20260914000001`). Los wizards mezclan `derivado < portal_data < borrador del wizard` (`ml-attributes` resuelve listas por nombre → id con `resolverIdsDeLista`, si no el select se veía vacío).
- **Landing en autopilot:** con las cuatro respuestas fijas (`preguntasFijasLanding`, ids `q1..q4` — por eso el fallback y la visita usan LAS MISMAS preguntas), `startCoCreation` marca `wizard_state.autopilot=true`; la etapa `avatars` genera con las respuestas y encadena `copy` (`etapaTrasAvatares`), y `LandingSection` publica sola (`publicarSola`) y muestra el enlace. Sin video/recorrido queda en borrador con el motivo. `autopilot` lo escribe SOLO el servidor (el PATCH del cliente lo descarta). Sigue siendo una llamada de IA por request.
- **Botón "Crear landing" de la tarjeta:** dispara la creación de `LandingSection` vía `onCrearLanding` → `autoStartToken` (antes linkeaba al editor, que sin landing redirigía a la ficha). La tarjeta se vuelve a montar (`key`) cuando la landing cambia (`onChanged`).
- **Stepper de los wizards:** `StepperPills` compartido + `puedeSaltarA` (`lib/portals/wizard-etapas.ts`): atrás siempre, adelante solo hasta la etapa más lejana alcanzada y con la actual válida; el salto guarda el borrador. `AttrField` también es compartido (ML, AP y la visita).
- **Seguridad (hallazgo del review):** `GET/PATCH /api/deals/[id]/visit-data` usa el cliente de servicio (sin RLS): la ruta es la única barrera y ahora exige `canAccessDeal` + abogado 403. Cualquier ruta nueva sobre `lib/supabase/visit-data` tiene que hacer lo mismo.
- **QA en la vista previa de Netlify:** el "Netlify Drawer" (iframe flotante) tapa los botones de abajo a la derecha y `click` no llega — sacarlo con `document.querySelectorAll('iframe[title="Netlify Drawer"]').forEach(e => e.remove())` o usar `form.requestSubmit()`. El modo prueba de email es un dato de la base (`notification_settings.test_mode_enabled`), no una env var: prenderlo para el QA y apagarlo al terminar.
...
- **Regla del guardado: al salir del campo (blur) o con Enter, NUNCA por tecla ni por temporizador.** Motivo concreto: para corregir 6 → 5 hay que vaciar el campo, y para escribir 1290000 se pasa por 1, 12, 129… Con la landing leyendo en vivo y tráfico pago encima, un autosave publicaría "US$ 12" o un dato en blanco. El primer diseño del panel tenía temporizador y los tests lo cazaron.

#### Las CINCO defensas del precio (cada una tapa un agujero real, encontrado por revisión adversarial)

El blur solo NO alcanzaba: quien tipea "12" de 1290000 y hace clic afuera publicaba US$ 12. Las defensas, en orden de fuerza:

1. **El campo muestra el número AGRUPADO mientras se escribe** (`lib/properties/money-input.ts`: 1.290.000, no 1290000) más una línea "Vas a publicar US$ …". Es la defensa más barata y la más efectiva: "12" y "1.290.000" dejan de parecerse y el error salta ANTES de guardar. Además tolera que se escriban los puntos de miles, que con `type="number"` producía basura.
2. **No se guarda tecleando**, sino al salir del campo o con Enter.
3. **Cambio brusco (>25%) o CUALQUIER cambio de moneda → confirmación** con los dos precios a la vista (`lib/properties/price-guard.ts`, puro). Una baja normal (4-15%) se guarda sola: un freno que salta siempre se vuelve ruido y se ignora.
4. **Mientras el cartel está abierto, el campo y el select quedan BLOQUEADOS.** ESTE ERA EL PEOR BUG: el cartel congelaba el valor del blur pero el campo seguía editable, así que corregir el número y apretar Confirmar publicaba el VIEJO (US$ 12) — el gesto más natural de recuperación era el que fallaba. El botón dice el monto exacto que va a publicar.
5. **El servidor repite el freno** (409 `requiereConfirmacion`): la confirmación corre en el navegador y un bug, una pestaña vieja o un llamado directo la saltean. La ruta relee el precio real de la base. La tarjeta ENTIENDE ese 409 y ofrece confirmar — si no, quedaba un callejón sin salida del que solo se salía recargando.

**Otras trampas que dejó la revisión adversarial (no repetirlas):**
- **El estado local se resincroniza con los props** (`useEffect` sobre `askingPrice`/`currency`). Sin eso, el campo conservaba el valor del montaje: si otra persona bajaba el precio, un clic adentro/afuera SIN escribir nada lo revertía en silencio (por debajo del 25% no saltaba ninguna alarma).
- **El techo del precio es POR MONEDA** (`TECHO_PRECIO` en editable-fields): 100M en USD, 200.000M en ARS. Con un techo único, una propiedad publicada en pesos quedaba imposible de editar —ni para BAJARLE el precio— con un mensaje que además mentía.
- **`sanearEdicion(body, monedaActual)`**: la ruta lee la propiedad ANTES de sanear, porque la moneda vigente define el techo.
- **El precio tiene UNA sola puerta.** El `PUT /api/properties/[id]` genérico ahora RECHAZA `asking_price`/`currency` con un 400 que apunta a `/details`; y los wizards de portales dejaron de reenviar `asking_price` (lo leían al abrirse y lo reescribían en cada guardado: bajar el precio en la ficha con el wizard abierto se revertía al avanzar un paso).
- **Límite conocido y aceptado:** si el precio nuevo queda CERCA del viejo, ninguna heurística de magnitud puede distinguir un error de un cambio legítimo (ej. propiedad en 130.000 y se tipea 129.000 queriendo 1.290.000). Contra eso juega la defensa 1: el número se ve agrupado y en limpio antes de guardar.
...
### Buscar texto en Postgres sin `unaccent`: `imatch` + clases de caracteres (2026-09-03)

- **Qué es:** el buscador de texto libre de los dos listados (Historial de Tasaciones y Propiedades) y su filtro de precio. Piezas: `lib/filters/busqueda-texto.ts` (puro, 40 pruebas) → `GET /api/appraisals` y `GET /api/properties` → `getPropertiesListPage`. Los controles: `components/filters/BusquedaTexto.tsx` y `RangoPrecio.tsx`, sobre `use-borrador-con-espera.ts` + `lib/filters/borrador-filtro.ts`.
- **Por qué NO `ilike`:** ignora mayúsculas pero NO tildes, y los datos mezclan las dos formas ("Diaz Velez" sin tilde, "Ciudad Autónoma" con tilde). La extensión `unaccent` **no está instalada** y PostgREST no sabe llamar funciones sobre columnas dentro de un `or()`. La salida sin tocar infraestructura es **`imatch`** (el `~*` de Postgres) con cada vocal convertida en clase: `d[iíìîï][aáàâäã]z`.
- **TRAMPA 1 — el texto de la persona NO es una expresión regular.** Sin escapar, buscar `2*D` devolvía **17 fichas en vez de 1** y un paréntesis suelto tiraba `400 invalid regular expression`.
- **TRAMPA 2 — dentro de comillas de PostgREST hay que DUPLICAR la barra invertida.** PostgREST usa `\` como su propio escape y se come una: `"2\*D"` llega a Postgres como `2*D` (otra vez 17 fichas); `"2\\*D"` llega como `2\*D` y devuelve 1. Y las comillas no son opcionales: una coma cruda rompe el árbol lógico (`PGRST100`). Son DOS capas de escapado: `patronRegex` produce barras simples, `valorPostgrest` las duplica — **no mezclarlas**.
- **TRAMPA 3 — macOS entrega el texto DESCOMPUESTO (NFD).** La "ó" copiada del Finder, de un PDF o de la terminal son DOS caracteres: `o` + U+0301. Sin `normalize('NFC')` el patrón exige esa tilde suelta literal y la búsqueda devuelve **cero fichas, sin error** — quien busca concluye que la propiedad no está cargada. Es el mismo U+0301 que rompe Turbopack por el nombre de esta carpeta.
- **TRAMPA 4 — el tope del TEXTO no acota la DIRECCIÓN.** Cada vocal se vuelve una clase de 6 caracteres que codificada pesa ~35, y cada palabra repite la condición para todas las columnas: el mismo tope de 80 caracteres produce direcciones de 1,4 KB a 15 KB. Medido: 33 palabras de una letra (65 caracteres, DENTRO del tope) → dirección de 10,9 KB → `UND_ERR_HEADERS_OVERFLOW` → **500**. Por eso el tope real (`MAX_CARACTERES_CONSULTA`) es sobre lo que pesa la consulta ya codificada, no sobre el texto. La primera palabra entra siempre: si la lista quedara vacía, buscar mostraría TODO, mentira peor que un error.
- **Varias palabras = Y de O:** una condición `or()` por palabra, y varios `.or()` sobre la misma consulta **se combinan con Y** (postgrest-js hace `searchParams.append`). Verificado: "almagro 3841" → 1 ficha, "almagro palermo" → 0.
- **`description` no se puede buscar** sin tocar `vw_properties_list`: no está en la vista.
- **Un control con espera necesita las TRES reglas juntas** (`lib/filters/borrador-filtro.ts`): el campo no se lee de la dirección (iría a los saltos), pero tampoco la ignora ("Limpiar todo", botón atrás), y lo que vuelve después de escribir es el ECO propio, no un cambio de afuera. Además **la espera pendiente hay que cancelarla cuando el filtro cambia desde afuera**: sin eso, escribir y tocar "Limpiar todo" hacía que la limpieza se deshiciera sola unos milisegundos después.
- **Verificación:** probe contra la API REAL (13 chequeos). Nada de esto se dedujo — todo se comprobó pegándole a Supabase.
...
## Descripciones de propiedades: el método de Diego (2026-09-19)

Spec: `docs/superpowers/specs/2026-09-19-descripciones-metodo-diego-design.md`. Plan: `docs/superpowers/plans/2026-09-19-descripciones-metodo-diego.md`.

- **Por qué se rehízo:** el generador viejo (un llamado a DeepSeek/gpt-4o-mini con 8 datos numéricos) escribía genérico e inventaba — Díaz Colodrero salió con "primer piso" (la ficha dice PB) y "un café en tu balcón" (no tiene: el asesor lo había escrito como objeción). Las REGLAS del GPT de Diego (`GPT Portales/`, 5 .docx) se habían copiado, pero se perdió la otra mitad del método: las preguntas (comprador ideal + Checklist), la búsqueda de la zona y los ejemplos reales.
- **Cómo funciona:** tres etapas, UN pedido por etapa y UNA llamada de IA por pedido (ver la regla dura de abajo): **fotos** (OpenAI `gpt-4.1` con visión, las primeras 30 fotos por URL, detalle `low`, salida con esquema JSON) → **zona** (mapa + web en paralelo) → **escribir** (`lib/descripcion/metodo-diego.ts`, los 5 documentos + los 3 ejemplos de Diego). Medido con propiedades reales: fotos 9–13 s, zona 3–9 s, escribir 4–6 s. Todo en `lib/descripcion/`; servicio con IO en `servicio.ts`; rutas `GET/POST /api/properties/[id]/descripcion` y `POST .../descripcion/guardar`; panel en `components/properties/descripcion/`. Probar sin guardar nada: `node --env-file=.env.local --import tsx scripts/descripcion-probar.ts <id o prefijo>`.
- **Dónde está el botón:** en la ficha, SIEMPRE (generar o regenerar), con vista previa editable; nada se guarda hasta "Guardar". Se sacó del alta a pedido del dueño: se genera con la propiedad cargada para que el sistema VEA las fotos. Los asistentes de MercadoLibre/Argenprop toman la descripción de la ficha y solo ofrecen generar si no hay (`DescripcionDelAviso.tsx`).
- **Las distancias salen del MAPA, nunca de la web:** dos búsquedas web sobre la misma dirección dieron 300 m y 800 m al mismo subte. El mapa es el **mapa propio** (ver la subsección de abajo): estaciones con sus líneas, plazas, colegios (sin los de adultos), hospitales y **colectivos que pasan a menos de 400 m** (la web decía "líneas que circulan por Almagro", que no es lo mismo). La web solo aporta carácter del barrio, comercios e hitos, y sus citas/links se limpian (`limpiar-web.ts`).
- **Preguntas UNA sola vez en toda la plataforma** (pedido del dueño): comprador ideal, diferencial, objeción y barrio son las mismas 4 de la visita (Sección 09) y de la landing. `respuestas.ts` junta lo ya contestado en `properties.landing_answers`, en la visita del proceso (`deals.visit_data.landing`) y en `property_landings.wizard_state` (ahí la IA inventaba las preguntas: se clasifican por TEXTO, con reglas calibradas con las reales). El panel pregunta solo lo que falta y lo guarda en `landing_answers` (solo claves vacías), así tampoco se pregunta al crear la landing.
- **Lo que NUNCA llega al prompt:** `reason_for_sale` y `sale_timeframe` de la visita (datos privados del dueño). La **objeción** llega solo como guarda ("no la menciones"). Todo texto de personas o de la web va entre « » sin « » internas.
- **Controles en código, no en el prompt:** disclaimer literal repuesto si el modelo lo cambia; adjetivos prohibidos, rótulos ("Primera parte:") y markdown detectados → el panel pide UNA corrección en otro pedido; aviso al asesor si las fotos muestran más dormitorios/baños que la ficha (`coherencia.ts`).
- **Hallazgo de datos (2026-09-19): Díaz Colodrero 2327 tiene cargadas las 20 fotos de Perón 4227 13° "B"** (huellas md5 idénticas), y su landing está publicada con ellas. No se corrigió: requiere OK del dueño. El aviso de coherencia lo detecta.
- **De paso se arreglaron dos piezas compartidas con el alta:** `parseAddress` no reconocía la altura con el piso pegado ("Perón 4227-13 \"B\"" → altura null) y el geocodificador ahora reintenta sin abreviaturas ("Tte. Gral. Juan D." no resuelve en OSM, "Juan Perón 4227" sí). Esa propiedad nunca se había ubicado en el mapa.
- **Caché:** `properties.descripcion_ia` (migración `20260919000001`, aplicada y verificada): inventario con firma de las fotos (cambia si cambia una URL o el orden), zona con firma de la dirección y el pin (`firmaZona`) y marca `completa` (desde el mapa propio, una zona sin mapa o sin web ya no se guarda: la etapa da error reintentable), notas del asesor y las últimas 3 descripciones reemplazadas (respaldo).
- **Guardar en una propiedad publicada actualiza los portales** (trigger `trg_requeue_listings_on_update`): es lo buscado; el panel lo avisa antes.
- **Variables:** `OPENAI_API_KEY` (ya estaba). Opcionales `DESCRIPCION_MODELO_FOTOS` / `DESCRIPCION_MODELO_TEXTO` (default `gpt-4.1`). Un modelo "con razonamiento" (gpt-5-mini) tardó 36 s en la zona: no sirve para estas etapas.
- **Decisiones del dueño del 2026-09-19 (después de probarlo en Perón 4227: "la diferencia es absurda a favor de la última versión"):**
  - NADA de regeneración en lote ni automática. Cada propiedad se regenera cuando alguien entra a su ficha y toca "Regenerar descripción", con el MISMO proceso completo. Se había diseñado un lote (cola + pg_cron + pantalla de revisión) y se descartó.
  - Las descripciones actuales NO son fuente: las escribió el sistema viejo, mucho más pobre. El proceso nuevo no las lee (hay un test en `entradas.test.ts` que lo garantiza); solo quedan como respaldo al guardar.
- **Terrenos:** piden la superficie del lote (`total_area`) en vez de ambientes y superficie cubierta, y tienen su estructura en el prompt (`TERRENO`, adaptación de la de casa sin recorrido — Diego no escribió una). Usos posibles (zonificación, metros construibles, pisos) SOLO si están cargados; lo obvio de lo que se ve (un lote en un barrio residencial sirve para una casa) sí se puede decir.
- **Escala:** índice `idx_deals_property_id` (migración `20260919000002`, aplicado y verificado con EXPLAIN): la ficha busca la visita del proceso por `deals.property_id` y no tenía índice. Lo demás que consulta la ficha ya estaba indexado; cada generación vive en la fila de su propiedad, nada global crece con la cantidad de propiedades.
- **Sigue vivo el generador viejo** (`lib/marketing/portal-descriptions/`) solo para `portal-description-bridge.ts` (landing/Meta generan al vuelo si la propiedad no tiene descripción). Ninguna pantalla lo usa.

### Mapa propio de la zona: 1.000 de 1.000 (2026-09-19)

Spec: `docs/superpowers/specs/2026-09-19-mapa-propio-design.md`. Motivo: Hipólito Yrigoyen 1550 salió "El mapa no respondió" y el dueño pidió que funcione "1.000 de cada 1.000 veces".

- **Por qué no Overpass en vivo:** los servidores públicos de OpenStreetMap fallaron 1 de 5 en producción (429, 504, 30 s sin respuesta; una vez los dos a la vez). Limitan por IP y Netlify comparte IP. Reintentar mejora pero nunca llega al 100 %.
- **Qué es:** los lugares del AMBA viven en NUESTRA base (PostGIS en el esquema `extensions`): `mapa_lugares` (~68.000 filas: estaciones, paradas, **tramos de recorrido de colectivo como LINESTRING**, plazas, colegios, universidades, hospitales) y `mapa_celdas` (grilla de 0,05°, **532 celdas**, del Gran La Plata a Pilar). La zona consulta la función `lugares_cercanos` (SECURITY INVOKER, solo `service_role`): medido 1.000 de 1.000 puntos al azar, ~0,5 s promedio y 0,9 s el peor, incluida la red. Migraciones `20260919000003`/`04` (aplicadas y verificadas).
- **Colectivos = paradas Y tramos a 400 m.** Solo con paradas faltaban líneas (Perón: la 19 y la 109), porque a muchas rutas de OSM les faltan las paradas pero nunca el trazado. Con los tramos, los colectivos coinciden 100 % con la consulta en vivo en las propiedades comparadas.
- **Pin fuera del recuadro → 409 "corregí la ubicación".** En la práctica es un pin mal puesto: **Almafuerte 2500 (San Martín) tiene el pin en Junín** (-34.58, -60.96). Falta corregirlo en la ficha. Y la zona guardada se rehace si cambia el pin (`firmaZona` = dirección + pin).
- **Nunca un texto sin mapa en silencio:** base caída → 503 (el panel reintenta solo), geocodificador lento → 503, 0 lugares y 0 colectivos → aviso visible al asesor. El mapa se consulta ANTES de pagar la búsqueda web.

**Carga y actualización:**
- **La carga completa (y la actualización confiable) es desde Geofabrik, NO desde Overpass.** La carga celda por celda contra Overpass terminó con la IP bloqueada. Receta (~20 min en esta Mac, `pip install osmium`):
  ```
  curl -o argentina-latest.osm.pbf https://download.geofabrik.de/south-america/argentina-latest.osm.pbf
  python3 scripts/mapa-extraer-osm.py argentina-latest.osm.pbf amba.jsonl
  node --env-file=.env.local --import tsx scripts/mapa-cargar-archivo.ts amba.jsonl
  node --env-file=.env.local --import tsx scripts/mapa-verificar.ts
  ```
  El cargador no toca la base si el archivo no termina en la marca de fin del extractor, o si alguna celda perdería más de la mitad de sus filas (`--forzar` solo sabiendo por qué).
- **Actualización mensual automática = un EXTRA:** pg_cron → `/api/cron/mapa-lugares` cada 5 min, UNA celda por corrida (la más vieja con más de 30 días). Overpass tarda 15–23 s en la celda más densa, y 11–19 s aun con un cuarto de celda (es el servidor, no el tamaño), así que las densas a veces no llegan en el tiempo de Netlify. **Nunca empeora los datos:** si falla, la celda conserva lo que tiene y espera 1 h; una descarga "buena" con menos de la mitad de lo que había cuenta como falla (espejo vacío o recortado); cada escritura reserva la celda (`reservada_hasta`, UPDATE condicional atómico) porque dos a la vez la vaciaban. Job: migración `20260919000005` con `scripts/apply-cron-mapa-lugares-pg.ts` (verifica `?ping=1` y reusa el secreto de `send_report` sin imprimirlo).

**La regla que no se puede romper: Python y TypeScript producen las MISMAS filas.** `scripts/mapa-extraer-osm.py` (carga) y `lib/mapa/overpass-celda.ts` (actualización) aplican las mismas reglas. Si difieren, cada actualización mensual borra o cambia datos buenos.
- Reglas compartidas: nombres (`nombreUtil`: sin caracteres de control y descarte de más de 120 — hay uno con los tags pegados), líneas de tren unidas (`lineaDeRuta`: "Ferrocarril Roca - Rápido" → "Línea Roca"), tipo de estación por sus RUTAS (`tipoDeEstacion`: Plaza Constitución dice "Subte" en la red pero sus rutas son del Roca), asignación de celda por `celdaDe` (un tramo, por su primer punto dentro del AMBA).
- **Verificar después de tocar cualquier regla:** `node --env-file=.env.local --import tsx scripts/mapa-comparar-celda.ts <archivo.jsonl> -34.650_-58.450` → tiene que decir "mismas filas". Verificado el 2026-09-19: 3.145 filas iguales en los 8 tipos.
- **Limitación conocida (aceptada):** un parque de forma rara cuyo centro de caja cae en una celda que el parque no toca se carga desde Geofabrik pero la actualización de esa celda no lo trae y lo borra. Es rarísimo; la próxima carga completa lo repone.
- **Trampa de Overpass:** responde **200 con un `remark`** cuando corta la consulta por tiempo o memoria, con `elements` vacío o a medias. Eso es una descarga fallida, no "no hay nada cerca" (`consultarServidorOverpass`). Y `overpass-api.de` sigue bloqueando la IP de esta Mac; el espejo `maps.mail.ru` responde.
...
### Publicar la landing: el gate protege del copy GENÉRICO, no exige un camino único (2026-08-18)

- **Symptom:** una landing creada, editada a mano y aprobada por el dueño (Coghlan) NO se podía publicar: "Antes de publicar, respondé todas las preguntas…". Las preguntas ni siquiera se ven desde el editor (viven en la ficha), así que desde ahí era un callejón sin salida cuya única salida ofrecida era BORRAR la landing y perder el trabajo. Y como publicar es lo que asigna `public_slug`, la campaña Meta decía "Primero creá la Landing Page" sobre una landing que existía.
- **Root cause:** el gate exigía el camino (a) —responder las preguntas + generar los textos con ellas— como ÚNICA forma de no ser genérico. Pero escribir los textos a mano en el editor cumple exactamente el mismo propósito. Verificado contra la base: la landing tenía `answers: {}`, `copyFromAnswers: false` y un `draft_content` DISTINTO del `content` (= edición manual real).
- **Fix:** `lib/landing/answers-gate.ts` expone `bloqueoDePublicacion(landing)` — única definición del gate, compartida por `publishLanding` y por la UI de la ficha. Devuelve `null` (publicable) si: ya está publicada, no hay preguntas, **fue editada a mano**, o se respondieron las preguntas y se generó el copy. `editadaAMano` compara `draft_content` vs `content` con serialización de claves ORDENADAS — abrir el editor sin tocar nada guarda un borrador idéntico y eso NO cuenta como escribir (comparar con `JSON.stringify` crudo abriría el gate solo por el orden de claves).
- **Mensaje:** dice DÓNDE están las preguntas y cuál es la alternativa. Un gate cuyo mensaje no dice cómo salir es un callejón sin salida.
- **Meta:** `meta-launch-v2/start` ahora distingue "no existe" de "existe sin publicar" (`landingExiste` en el 409) y el wizard muestra "Falta publicar la Landing Page" con el botón correcto. Mandar a *crear* a quien ya creó lo empuja a borrar y perder su trabajo.
...
### "Modificar ficha" es UNA sola puerta — no volver a repartirla (2026-08-25)

- **Qué es:** el panel `components/properties/detail/PropertyDetailsEditor.tsx` ("Modificar ficha", en la pestaña Propiedad) es el ÚNICO lugar para corregir los datos de una propiedad ya cargada: **tipo, operación, precio + moneda, comisión, fechas de contrato, ambientes, dormitorios, baños, cocheras, superficies, antigüedad, piso, expensas y descripción**.
- **POR QUÉ ESTÁ TODO JUNTO (la razón de la regla):** esto vivía repartido en editores sueltos por la ficha y **se rompió sin que nadie lo notara**: un cambio que agregó el editor de tipo dejó de renderizar el de características y el de precio, y la ficha volvió a ser de solo-lectura. Una sola puerta es más difícil de romper en silencio. **Antes de sacar un campo de este panel o de partirlo en pedazos, entender que ese fue exactamente el fallo anterior.**
- **Guardado: al SALIR de cada campo, nunca por temporizador.** Para corregir 6 → 5 hay que vaciar el campo, y un guardado por tiempo puede dispararse justo ahí y dejar el dato en blanco. La landing pública lee estos valores en vivo: ese instante se ve. Los desplegables sí guardan al elegir (no tienen estado intermedio).
- **El precio conserva sus cinco defensas** dentro del panel (ver la sección de arriba): se escribe agrupado en miles, un cambio brusco o de moneda pide confirmación con los dos importes a la vista, y mientras el cartel está abierto el campo queda BLOQUEADO. El servidor repite el freno con 409 y el panel lo entiende y ofrece confirmar.
- **Todo pasa por `PATCH /api/properties/[id]/details`** con la lista blanca de `lib/properties/editable-fields.ts`. Lo que no está en esa lista no viaja: el `PUT` genérico manda el body entero al UPDATE, así que editar desde el navegador podría colar `legal_status`, `assigned_to` o `status`. **Al sumar un campo editable hay que tocar los DOS lados:** la lista blanca (con su validación) y el panel.
- **`validarRangoDeContrato` va aparte de `sanearEdicion`** porque necesita las dos fechas: si el formulario manda solo una, la otra la aporta la ruta, que es la única que conoce el valor vigente.
- **Efecto en la landing:** los datos se actualizan solos donde la landing los lee en vivo; los textos que ya escribió la IA NO se reescriben. Los avisos ya publicados en portales no se actualizan solos (decisión tomada, no es un bug).
- **`PropertyTypeEditor` quedó huérfano a propósito** (el tipo se edita en el panel). El componente y su endpoint siguen en el repo y funcionan; no se borraron por si hace falta cambiar solo el tipo desde otro lado.

### Tasador IA — segunda opinión de precio con el MISMO método (2026-09-10)

- **Qué es:** cada tasación puede tener dos valuaciones: la clásica (`valuation_result`, la de siempre) y la del Tasador IA (`ai_valuation_result`). **La IA es el tasador, la calculadora es la misma:** el modelo devuelve calidad, estado de conservación, disposición, piso, antigüedad y coeficiente de ubicación por propiedad (validado con zod, enums exactos, J en [0.70, 1.30]; cualquier violación = `failed`, nunca datos a medias) y `calculateValuation` saca los números.
- **REGLA DEL DUEÑO (2026-09-12): lo que cargó el asesor MANDA al 100%**, en la propiedad tasada y en cada comparable. `fusionarInterpretacion` conserva calidad/estado/disposición/J/piso/antigüedad si están cargados y toma de la IA SOLO lo vacío (en la práctica: el coeficiente de ubicación, que casi nunca se carga, y piso/antigüedad faltantes). El prompt manda `fijadoPorElAsesor` + `aDecidir` por propiedad; si la IA no coincide con un valor fijo, lo dice en la justificación pero no lo cambia. Por eso la huella (`huellaDeInsumos`) INCLUYE esos juicios: cambiar la calidad de un comparable deja la IA "Desactualizada". **Trampa del método que confundió al dueño:** bajar la calidad/estado/ubicación de un COMPARABLE SUBE el precio de la tasada (precio ajustado = precio/m² ÷ N; N más chico → ajustado más alto); bajar la de la TASADA lo baja. No es error de cálculo, es el método. Regenerar todas tras un cambio de regla: `scripts/generar-tasador-ia-historico.ts --todas` (no toca `valuation_source`). Por eso el resultado IA tiene EXACTAMENTE la forma `ValuationResult` y tablas, Mapa de Valor, gastos, escenarios y PDF no distinguen de dónde salió. Spec: `docs/superpowers/specs/2026-09-10-tasador-ia-design.md`.
- **Piezas:** `lib/valuation/tasador-ia.ts` (prompt generado desde `VALUATION_RULES`, validación, `correrTasadorIA` con `chat` inyectable) · `completar-valuacion.ts` (parte del propietario + escenarios, extraída del wizard para que las dos versiones sean comparables) · `huella-insumos.ts` · `valuacion-activa.ts` (`valuacionActiva`, `estadoTarjetaIA`, `preciosDesnormalizados`) · `editar-snapshot-ia.ts` · `components/appraisal/SelectorDeTasador.tsx` · `POST /api/appraisals/[id]/ai-valuation` · `PATCH /api/appraisals/[id]` con `{valuationSource}` o `{aiValuationResult}`.
- **Columnas (migración `20260910000001`, aplicada):** `ai_valuation_result`, `ai_valuation_status` (`pending|ready|failed`), `ai_valuation_error`, `valuation_source` (`calculator|ai`, default `calculator`).
- **Los tres precios desnormalizados SIGUEN al tasador elegido.** `publication_price`, `sale_value` y `money_in_hand` se reescriben solo desde el tasador en uso (`elegirTasador`, `guardarValuacionIA`, y `replaceAppraisalComparables` los omite si la IA está elegida). Así los 12 lectores de `publication_price` (listado, pipeline, contactos, emails, adjunto PDF, captación, `appraisal-access`) siguen la elección **sin cambios**. Si agregás un consumidor nuevo, leé la columna, no el JSONB.
- **Dos snapshots independientes:** editar en línea con la IA elegida (detalle) recalcula SOLO `ai_valuation_result` (features en `ai.comparables[i].features` / `ai.subject.features`); la clásica no se entera, y viceversa. **El wizard NO edita la versión IA** (en modo IA el informe es de solo lectura con aviso): el wizard edita las FILAS y su efecto de recálculo reescribe la clásica; mezclar los dos caminos ahí duplicaba riesgo en un archivo de 1.700 líneas.
- **Huella (`huellaDeInsumos`):** hash cyrb53 de los insumos OBJETIVOS (precios, superficies, descripciones, ubicación, piso, antigüedad, tasas, parte del propietario). NO incluye calidad/estado/disposición/J — eso es lo que cada tasador decide. Huella actual ≠ `ai.inputFingerprint` → tarjeta "Desactualizada" + Regenerar. Se regenera sola SOLO al tocar "Calcular" en el wizard (no en cada autosave: sería una llamada paga por tecla). Corre en servidor y navegador: nada de `node:crypto`.
- **Al cambiar de tasador se BORRAN `report_edits.priceOverrides`** (fueron editados contra los números del otro) y la pantalla lo avisa. El resto de `report_edits` se conserva.
- **Netlify:** una llamada al modelo por request, `timeoutMs` 20 s, `maxTokens` 2000, temperatura 0.2, sin `maxDuration`. Medido el 2026-09-10 sobre una tasación real de 4 comparables: **6,7 s, 2.532 tokens (2.029 entrada / 503 salida), `gpt-4o-mini`**. Un `pending` con `updated_at` de más de 2 minutos se trata como fallido (la función murió a mitad). `TimeoutError` se traduce a "el modelo tardó más de 20 segundos".
- **Proveedor:** `chatCompletion` elige DeepSeek si hay `DEEPSEEK_API_KEY`, si no OpenAI (`gpt-4o-mini` o `AI_MODEL`). Override opcional: `TASADOR_IA_MODEL`. Sin ninguna clave, la ruta responde 503 y la tarjeta dice "Tasador IA no configurado" — nada más falla.
- **Permisos:** mismos dos candados que el PUT (`puedeEditarTasacion` + `canAccessAppraisal`). El abogado recibe la ficha resumida (`COLUMNAS_TASACION_RESUMIDA`), que NO incluye el resultado IA, y ve los importes del tasador elegido.
- **Probar sin UI:** `node --env-file=.env.local --import tsx scripts/probar-tasador-ia.ts <appraisalId>` (sin id lista candidatas; **no escribe**). Tests: `npx vitest run --config vitest.tasador-ia.config.ts` (~4 s). Estados del selector: `node --import tsx scripts/selector-de-tasador.probe.tsx`.
- **Detection:** `SELECT ai_valuation_status, count(*) FROM appraisals GROUP BY 1;` — muchos `failed` = cambió el modelo o el prompt (mirar `ai_valuation_error`); `pending` viejos = la función se corta (bajar `MAX_DESCRIPCION`/`maxTokens` en `tasador-ia.ts`). Si el listado muestra un precio que no coincide con el informe, mirar `valuation_source`: los precios desnormalizados deben ser los del tasador elegido.
...
### Teléfonos: NUNCA igualdad exacta — últimos 10 dígitos (`phone_norm`) (2026-08-16)

- **Symptom:** un cliente real de campaña respondió a la plantilla de tasación ("Coordinar por acá", después "Prefiero que me llamen") y el agente NO le contestó nada. El interruptor estaba prendido y el análisis corría.
- **Root cause — el "9" argentino:** el mismo número vive en la base en TRES formatos: `+5491149372737` (E.164 móvil: formulario/WhatsApp), `+541149372737` (sin el 9: imports viejos/GHL), `1149372737` (pelado: CSV). El contacto era de mayo (sin 9); WhatsApp reporta con 9; `buscarTratoDeTasacion` comparaba con `.in('phone', variantes)` exacto → no encontró el trato → silencio. El dedup exacto ADEMÁS venía duplicando contactos (Ada/Susana/Eduardo, verificado: la misma persona dos veces).
- **Fix:** clave canónica = **últimos 10 dígitos** (área+número, inmune a +54/9/espacios). Vive en DOS espejos declarados: `lib/phone/ultimos-digitos.ts` (TS, testeado) y la columna **GENERADA** `contacts.phone_norm` + índice (migración `20260816000001`, aplicada). Usada en: agente de tasación, dedup del embudo (`create-funnel-lead`), y Coordinar Tasación (`/api/deals`). Al reusar un contacto con teléfono distinto al recién tipeado se ACTUALIZA al nuevo (auto-curación de los sin-9). Verificado contra la base real: los 3 formatos de Daniel convergen; colisiones = solo duplicados de la misma persona.
- **Regla:** cualquier lookup nuevo de teléfono usa `phone_norm`/`ultimos10Digitos`, jamás `.eq('phone', ...)`. Si otra tabla necesita buscarse por teléfono a escala, se le agrega su columna generada (patrón de esta migración).

### El webhook de WhatsApp es idempotente por wa_message_id — también el PIPELINE (2026-08-16)

- El upsert por `wa_message_id` protegía la tabla pero NO el pipeline: un reintento de Meta re-corría el agente → respuesta duplicada al cliente. Ahora `persistInbound` devuelve `esNuevo` (el `.select('id')` del upsert con `ignoreDuplicates` devuelve fila solo si insertó) y el pipeline de IA corre SOLO sobre mensajes nuevos. Ante error del insert se asume nuevo (mejor doble respuesta que cliente sin atender).

### Worker del embudo: el trabajo 'whatsapp' tiene ventana PRIORITARIA (2026-08-16)

- El worker (`side-effects-worker.ts`) toma DOS ventanas: primero los `whatsapp` pendientes, después el resto. Sin esto, con backlog (pico de registros o un tercero caído acumulando reintentos) los avisos internos viejos iban por delante y el primer mensaje al cliente —promesa "en los próximos segundos" + apertura de la ventana de 24h— se demoraba decenas de minutos. Bajo carga se atrasan los avisos internos, nunca el saludo al cliente.
- **Auditoría de escala 1000 registros/día (2026-08-16, 20 agentes)** — estado por riesgo:
  - **(RESUELTO 16/08) Monitoreo out-of-band:** `funnel_watchdog_state` + ruta `/api/cron/funnel-watchdog` + job pg_cron `funnel-watchdog` (*/10). Si hay pendientes con >10 min de atraso → email a admins/dueños (dedup 6 h vía `last_alert_at`). `last_check_at` = latido del vigilante. Límite aceptado: si pg_cron entero muere, muere con él.
  - **(RESUELTO 16/08) Tier de WhatsApp:** los códigos 131048/131056/130429/80007 se marcan con `MARCA_LIMITE_DE_VOLUMEN` (jobs-logic) y el worker reintenta cada 4 h (`ESPERA_LIMITE_SEGUNDOS`) en vez de la escalera — 5 intentos cubren ~20 h, dentro de la ventana en que el cupo del tier se renueva.
  - **(PENDIENTE) Throughput del worker:** ~6-8 trabajos/min serial → en picos de 100 registros/hora los avisos internos se atrasan (el WhatsApp al cliente NO: tiene ventana prioritaria).
...
- **Usuario "Claude · pruebas"** (rol `admin`, activo) creado el 2026-09-10 con `scripts/crear-usuario-claude-qa.ts` (idempotente); sus datos de acceso están en `.env.local`, fuera de git. Todo lo que Claude hace en la plataforma queda a nombre de ese usuario.
- **Entrar al navegador de Claude (verificado 2026-09-10):** `node --env-file=.env.local --import tsx scripts/navegador-claude-login.ts`. Canjea un magic link en el servidor (`verifyOtp` con `hashed_token`) y escribe la sesión en las cookies del navegador por CDP con el formato de `@supabase/ssr`; antes **borra las cookies previas** (el dueño se había logueado a mano en esa ventana y el QA habría corrido con SU usuario, no con el de pruebas). Termina verificando que `/inicio` no rebote a `/login` y que la cookie sea de `claude.qa@inmodf.com.ar`. **Lo que NO anda y por qué:** el flujo normal del magic link redirige a `http://localhost:3000` (Site URL del proyecto; inmodf.com.ar no está en las redirecciones permitidas de Supabase Auth), y `/login#access_token=…` no sirve porque la pantalla crea el cliente de Supabase recién al enviar el formulario. Tipear la contraseña desde un script lo bloquea el clasificador de permisos.
- **Push, PR y merge a `main` los hace Claude, nunca el dueño.** En la Mac hay DOS cuentas de GitHub en `gh`: `Sujupar97` (sin permisos) y `sujupar` (admin del repo). Si `gh api user -q .login` devuelve `Sujupar97`, correr `gh auth switch --user sujupar` (así se resolvió el 2026-09-10 el "must be a collaborator" de `gh pr create`). Las ramas sin PR NO se publican en Netlify (404 verificado): siempre abrir el PR para tener la vista previa.
...
### A/B de `/tasacion-directa`: "siempre me abre la B" NO es un bug del reparto (2026-09-19)

- **Symptom:** el dueño abre `inmobiliariadiegoferreyra.com/tasacion-directa` y ve SIEMPRE la variante B; concluye que todo el tráfico va a la B.
- **Root cause (del síntoma):** hasta el 2026-09-19 el reparto era por VISITANTE: el middleware dejaba un número 0–999 en la cookie `df_lp_roll` por 90 días y la página decidía con `roll*100 < split_b`. Quien había sacado un número < 500 veía la B cada vez que entraba desde ese navegador. Repartía bien (verificado: 30 `curl` sin cookies → corte exacto en 500), pero para el dueño "no funcionaba".
- **DECISIÓN DEL DUEÑO (2026-09-19, reafirmada tras escuchar el reparo): el reparto es POR CLIC**, como el split test de GoHighLevel. Cada apertura se sortea de nuevo: `variantePorClic()` en `lib/funnel/ab-test.ts`, llamada desde `page.tsx`. Ya no hay cookie ni participa el middleware. **No volver al reparto por visitante sin que él lo pida.** Lo que se cede, medido ese día: el 17,6% de las personas entra más de una vez (20% de las visitas son re-visitas) y puede ver las dos versiones; un registro se anota a la versión donde se llenó el formulario. La medición sigue siendo honesta POR PÁGINA SERVIDA porque `landingVariant` viaja fijo en cada landing (submit y `LandingVisitTracker`), no sale de ninguna cookie.
- **TRAMPA del reparto por clic — la caché:** el sorteo ocurre en el servidor en cada pedido. Si la página se cacheara (Next o la CDN de Netlify), TODOS verían la variante del primer pedido y el test quedaría 100/0 **sin ningún error**. Antes lo evitaba de rebote la lectura de `cookies()`; ahora lo garantiza `export const dynamic = 'force-dynamic'` en `page.tsx` — NO sacarlo. `getExperiment` puede cachear la CONFIG, nunca la variante.
- **Cómo verificar el reparto sin opinar:** N `curl` CON el mismo tarro de cookies (`-b jar -c jar`, o sea "la misma persona") leyendo el `landingVariant` del HTML → tienen que salir las dos variantes cerca del porcentaje del panel; y `curl -I` → `cache-control: private, no-cache, no-store` y ningún `set-cookie: df_lp_roll`. Para ver una variante puntual: `?lp=A` / `?lp=B` (OJO: esa visita se registra con esa variante). Datos reales: `landing_page_visits.landing_variant` por día, filtrando `is_bot`.
- **El bug REAL que había detrás de "no convierte":** en `TasacionNetaClient.tsx` el botón verde (`Cta`) estaba declarado DENTRO del render. El primer gesto llama a `prime` → cambia estado → re-render → `Cta` es un tipo NUEVO → React desmonta y remonta el botón en pleno toque → el clic se pierde. En compu el hover precarga antes y anda; **en celular los dos botones pedían DOS toques**. Del 15 al 19/9: 59 visitas a la B (86% celular), 0 registros reales. Fix: `Cta` a nivel de módulo, como en la A. Test: `TasacionNetaClient.test.tsx` (nodo del DOM idéntico tras `mouseEnter` + abre al primer clic).
- **Regla general:** NUNCA declarar un componente dentro de otro (`const X = () => <…/>` adentro del render). Cualquier cambio de estado del padre lo remonta: se pierde foco, clics en curso y estado interno. Si necesita datos del padre, van por props.
- **Detection:** `click` del MCP y el mouse NO lo reproducen (el hover precarga). Hay que mandar un toque táctil real por CDP (`Emulation.setTouchEmulationEnabled` + `Input.dispatchTouchEvent` touchStart/touchEnd) y mirar si aparece `[role="dialog"]` al PRIMER toque. Bloquear `*/api/*` y `*facebook*` con `Network.setBlockedURLs` para no registrar visitas ni eventos del Píxel durante el QA.
- **Caída de tráfico del 16/9 (no es de la plataforma):** con el mismo gasto (~$12.500/día) Meta pasó de 55–100 clics/día a 27–47 y de ~20 visitas a la página a 3–12. `meta_ads_daily` y `landing_page_visits` coinciden → la medición está bien; es entrega de Meta (mirar en Ads Manager qué cambió el 14–16/9; el 14/9 hubo una tormenta de robots: 201 marcados + ~230 visitas "humanas" anómalas).

### Mapa de calor: el dato se guardaba y ninguna pantalla lo pedía (landing B, 2026-09-19)

- **Symptom:** la landing B registraba su calor como `tasacion-neta` desde el 15/9 (59 sesiones, 50 clics) y no había forma de verlo. Nada fallaba.
- **Root cause:** la lista de "páginas con calor" estaba copiada a mano en CUATRO archivos (API de lectura, página del visor, panel resumen, API de métricas) y en ninguno figuraba la B. La escritura (`POST /api/track/heatmap`) acepta cualquier `page`. Otra vez "se cambió el productor y se olvidó el consumidor".
- **Fix:** catálogo ÚNICO en `lib/funnel/heatmap-pages.ts` (página, embudo, variante, secciones, `heatmapPreviewSrc`). Lo leen los cuatro. `heatmap-pages.test.ts` lo compara contra el CÓDIGO de las landings: recorre `app/(funnels)` entera buscando `<FunnelHeatmapTracker page=…>` y exige que cada nombre y cada `data-hm` (en orden, sin contar comentarios) estén en el catálogo. **Al agregar una landing o una sección, esa prueba se pone roja: se arregla sumándola al catálogo, no aflojando la prueba.**
- **Dónde se ve:** Embudos → detalle de Tasación → tabla de calor con selector Versión A / Versión B (`HeatmapPanel`, nunca suma una con la otra), y el visor `/embudos/heatmap/tasacion` | `/embudos/heatmap/tasacion-neta` con interruptor. El visor arma el iframe con `hm_preview=1` (no registra visita, calor, video ni Píxel — verificado con la red) y `lp=A|B` (el A/B reparte por clic: sin forzar, saldría una versión al azar bajo el calor de la otra).
- **Tres defectos VIEJOS de la medición que el selector A/B dejó a la vista** (los encontró la revisión adversarial; los tres hacían engañosa la comparación):
  1. **Clics multiplicados por etapa (SQL).** `heatmap_section_stats` agrupaba las secciones por etapa pero contaba los clics SIN etapa, y el join tampoco la miraba → cada fila de etapa de los "registrados" repetía el total. Las pantallas suman filas: la A mostraba **720 clics contra 390 reales**; la B daba bien solo porque todos sus registrados estaban en una etapa. Migración `20260919000003_heatmap_clics_por_etapa.sql` (clics con etapa + `IS NOT DISTINCT FROM`, porque la etapa de los anónimos es NULL). Aplicada y verificada con `scripts/apply-heatmap-clics-por-etapa-pg.ts`: las tres páginas coinciden exacto con `heatmap_clicks`. **Regla general:** si una consulta agrupa por una columna y le pega un agregado calculado SIN esa columna, y después alguien SUMA las filas, el agregado sale multiplicado.
  2. **"Sección vista" imposible para secciones altas (tracker).** La regla era `intersectionRatio >= 0.5`; una sección más alta que dos pantallas nunca muestra su mitad. "Testimonios" (2.110 px en celular) daba 0 de 829 en la A y 0 de 50 en la B, con clics adentro y con el botón de MÁS ABAJO mostrando llegada. Ahora: media sección **o** media pantalla (`lib/funnel/heatmap-visibility.ts`, 6 pruebas) y umbrales cada 5% (con `[0, 0.5, 1]` el navegador no avisaba a mitad de una sección alta). **Las sesiones anteriores al 19/9 quedaron medidas con la regla vieja.**
  3. **Sección dentro de sección (landing B).** `data-hm="hero"` estaba en el `<section>` y envolvía a `video`, `qualifier` y `cta-1`. El bloque medía 1.133 px en celular → el "hero" de la B parecía perder 18 puntos más que el de la A, y la caída hero → video del panel no significaba nada. Ahora envuelve solo título y texto (537 px), en un `<div>` sin clases: maquetación verificada IDÉNTICA contra producción en 390 y 1440 px. En la B esa fila se llama "Título y texto" (`sectionLabel(section, page)`), porque ahí el video es otra sección.
  - Y uno del visor: los clics SIN sección (los del formulario, que se abre por fuera de toda `data-hm`) caen todos en una celda central "fantasma" (116 clics en la A contra 7 de la celda real más caliente). El overlay no la dibuja pero calculaba la intensidad contra ella → ninguna mancha pasaba de naranja pálido. El visor ahora la descarta.
- **Cómo probar el tracker SIN escribir en la base** (la vista previa usa el Supabase de producción): `Page.addScriptToEvaluateOnNewDocument` que reemplaza `fetch` y `navigator.sendBeacon`, guarda el body en `window.__capturados` y devuelve un OK falso; emular celular, `Page.bringToFront` (en segundo plano el IntersectionObserver no dispara), recorrer la página de a ~280 px y disparar `pagehide`. Así se vio "Testimonios: NO vista" en producción y "vista" en la vista previa.
- **Pendientes que dejó esa revisión:** (a), (b) y (c) se hicieron el mismo día (sección siguiente). Queda (d): `pageHeatGrid` viaja en `/api/funnels/metrics` y nadie lo consume (peso muerto, sin riesgo).

### Video de A y B mezclado, formulario vivo dentro del visor, y el tope de 1000 filas (2026-09-19, PR #22)

- **Video — Symptom:** Embudos mostraba UN solo bloque "Video del hero" para tasación, con la retención de las dos landings mezclada. **Root cause:** A y B usaban el mismo `trackKey="hero-tasacion"` para dos videos distintos (196 s y 711 s): la B se armó copiando la A. La tabla es `unique (anon_id, video_key)` y Embudos arma un bloque por clave. **Fix:** la B usa `hero-tasacion-neta`; catálogo único `lib/funnel/video-keys.ts` (nombre + orden, A antes que B). Su prueba recorre `app/(funnels)` y falla si dos landings comparten `trackKey` o si una clave no está en el catálogo. **Al copiar una landing para hacer otra versión, cambiarle el `trackKey` y el `page` del `FunnelHeatmapTracker`: son la identidad de sus métricas.**
- **Lo ya mezclado se separó por la DURACIÓN del video**, no adivinando: migración `20260919000004_video_key_landing_b.sql` (`duration_s > 400`, acotada a `first_at < 2026-09-21`), aplicada con `scripts/apply-video-key-landing-b-pg.ts` DENTRO de una transacción (si la verificación no cuadra hace ROLLBACK). 90 filas antes y después; 9 pasaron a la B. Límites conocidos, escritos en la migración: `duration_s` guarda la ÚLTIMA duración reportada (no la máxima), así que 1 de las 9 es en realidad una vista de la A de julio pisada por la B (un contacto registrado: alguien del equipo probando); y una fila escrita con la clave vieja entre la migración y el deploy queda trabada bajo la A por el UNIQUE — se detecta con `select count(*) from video_view_state where video_key='hero-tasacion' and duration_s > 400` (tiene que dar 0).
- **Formulario en el visor — Symptom:** el visor del mapa de calor embebe la landing REAL con `?hm_preview=1`. Eso apagaba visita, calor, video y Píxel, pero el formulario seguía vivo: llenarlo "para probar" creaba un lead real + aviso al equipo + conversión a Meta + un registro en el A/B. **Fix (dos barreras):** `FunnelLeadForm` corta al inicio de `handleSubmit` con un mensaje; `POST /api/funnel/submit` rechaza con 400 si `eventSourceUrl` o el `Referer` son del visor, ANTES de la reserva, y **registra el rechazo** (`motivo: 'visor_mapa_calor'`) como exige esa ruta desde el 14/8. El visor tiene el enlace "Abrir la landing real ↗" (`heatmapRealHref`, sin `hm_preview`).
  - **Es la única excepción a "un metadato de tracking jamás voltea una conversión"** (`submit-schema.ts`). El único falso positivo posible es que alguien comparta o ponga en un anuncio la dirección del iframe (clic derecho → "abrir marco en pestaña nueva" CONSERVA `hm_preview=1`). Por eso el rechazo se registra con los datos de contacto: ese lead se ve en `funnel_submit_rejections` y se recupera a mano. **Nunca usar una dirección con `hm_preview=1` fuera del visor.**
  - `isHeatmapPreview()` (navegador) ahora delega en `esVistaPreviaDelMapa(url)` (pura, parsea la URL): antes era una regex suelta que también saltaba con `?hm_preview=11` o `?next=/y?hm_preview=1`.
  - **Cómo se probó sin crear leads:** en el navegador, `Network.setBlockedURLs(['*/api/funnel/submit*'])` durante todo el QA (dentro del visor: 0 pedidos; visita normal: 1 pedido, frenado por el bloqueo). Contra el servidor: un envío SIN email ni teléfono, que rebota igual más adelante — si responde el mensaje del mapa de calor, la barrera actuó primero.
- **Tope de 1000 filas de PostgREST — Symptom (medido, no supuesto):** con un rango de 90 días `funnel_video_heatmap` devolvía 1000 de 1300 filas, status 206, sin error; las que se caían eran TODAS las del video de la B → "Vistas: 9" con la curva de retención plana en 0 %. A 30 días quedaba margen para 3 combinaciones (video, segmento, etapa) más. **Root cause:** `safeRpc` hacía una sola llamada sin orden ni paginación y filtraba en JavaScript. **Fix:** `lib/supabase/rpc-paginado.ts` (páginas de 1000, orden OBLIGATORIO por las columnas que identifican la fila, techo de páginas que avisa `truncado`). Lo usan las 8 RPC de `/api/funnels/metrics` y las 3 de `/api/funnels/heatmap`. **Regla general:** toda RPC o `select` que pueda pasar de 1000 filas se pagina CON orden; sin orden fijo, paginar repite o saltea filas.
- **De paso:** `FunnelLeadModal.test.tsx` estaba ROTO en `main` (esperaba textos que el formulario ya no tiene y no esperaba el envío asíncrono). Nadie lo corría. Y las pruebas del formulario no pueden afirmar "no envió" con un `setTimeout`: el envío carga `libphonenumber` de forma asíncrona. Lo firme es que nunca aparezca "Enviando...", que se pone de forma sincrónica.
- **Dato para leer el video con cuidado (no se tocó):** 37 de 81 "vistas" de la A y 4 de 9 de la B son filas con `watch_seconds=1`, `play_intents=0`, `playback_started=false`: inflan "Vistas" y bajan la profundidad media en las dos versiones por igual.
