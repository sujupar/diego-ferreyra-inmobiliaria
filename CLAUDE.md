...
- **Qué es:** cada tasación puede tener dos valuaciones: la clásica (`valuation_result`, la de siempre) y la del Tasador IA (`ai_valuation_result`). **La IA es el tasador, la calculadora es la misma:** el modelo devuelve calidad, estado de conservación, disposición, piso, antigüedad y coeficiente de ubicación por propiedad (validado con zod, enums exactos, J en [0.70, 1.30]; cualquier violación = `failed`, nunca datos a medias) y `calculateValuation` saca los números. Por eso el resultado IA tiene EXACTAMENTE la forma `ValuationResult` y tablas, Mapa de Valor, gastos, escenarios y PDF no distinguen de dónde salió. Spec: `docs/superpowers/specs/2026-09-10-tasador-ia-design.md`.
