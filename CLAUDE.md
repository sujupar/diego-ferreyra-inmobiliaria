...
- **Auditoría de escala 1000 registros/día (2026-08-16, 20 agentes):** riesgos CONFIRMADOS y pendientes: (1) throughput del worker ~6-8 trabajos/min serial → en picos de 100 registros/hora la cola crece (mitigado para el cliente por la ventana prioritaria; los avisos internos igual se atrasan); (2) CERO monitoreo out-of-band de la cola (`funnel_lead_jobs` pending envejecidos) ni del cron muerto — ya costó 6h de ceguera el 13/08; (3) tier de mensajería de WhatsApp (250→1K→10K/día): el código NO distingue los errores 131048/131056 y un rebote por tier agota los 5 reintentos (~7,5h) y escala como fallo genérico; (4) Resend: 2 emails/registro = 60k/mes a 1000/día (sobre el plan Pro) sin manejo de 429, y el canal de ALERTA es el mismo Resend que falla; (5) IA = DeepSeek único sin retry/failover (env + redeploy manual). Nada de esto muerde a volumen actual; TODO esto muerde antes de 1000/día.
