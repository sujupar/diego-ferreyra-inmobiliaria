...
- **Fuentes:** JSON Bryn (precio 48 barrios + kpis; fallback: data-* del SVG del mapa), RSS Colegio de Escribanos (escrituras + imagen a Storage `market-data/escrituras/{period}.jpg`), Zonaprop `/barrios/capital-federal/{slug}` vía ScraperAPI (6 conteos). **Infogram (composición del stock) DIFERIDA:** el embed NO trae los datos en `window.infographicData` como se planeó — son 24 charts "live" (`chartData.custom.live.provider === 'atlas_google_drive'`) con `chartData.data` siempre vacío; la hidratación real ocurre client-side contra `GET https://e.infogram.com/api/v1/atlas/getLiveData?id={live.key}`, que devuelve **401 sin sesión de browser** (no es un problema de parseo/selector). `lib/market-data/sources/infogram.ts` es un stub honesto que siempre devuelve `ok:false` con motivo accionable y NUNCA fabrica datos ni pega a la red para intentarlo. La sección Stock del PDF cae al override manual (imagen legacy) mientras esto siga diferido. Caminos de desbloqueo documentados en la cabecera del archivo: (1) ScraperAPI con `render=true` para ejecutar el JS del embed antes de devolver el HTML, o (2) acceso directo de Daniel Bryn a la hoja de Google Sheets que alimenta los charts.
- **Zonaprop sin verificar contra HTML real:** el parser (`lib/market-data/sources/zonaprop.ts`) está implementado pero NUNCA se contrastó contra HTML real de Zonaprop — ScraperAPI se quedó sin créditos durante la implementación (2026-07). Pendiente cuando se renueven los créditos: capturar un fixture real, adaptar selectores si hace falta, y correr `scripts/verify-zonaprop-slugs.ts` (verifica los 48 `zonapropSlug` del catálogo).
