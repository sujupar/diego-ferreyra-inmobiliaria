...
- **Default = `free` (publicación gratuita).** Decisión del usuario (2026-06-06): el wizard publica gratis por defecto, sin cupo ni costo. `gold_premium`/`silver` quedan como opciones que el asesor puede elegir en el paso 3 (y requieren cupo pago en la cuenta de ML; si no hay cupo, el fallback degrada solo).
