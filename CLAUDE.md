...
- **Root cause (del síntoma):** el reparto es por VISITANTE, no por clic. El middleware deja un número al azar 0–999 en la cookie `df_lp_roll` por 90 días y la página decide con `roll*100 < split_b`. Quien sacó un número < 500 ve la B cada vez que entra desde ese navegador. Es a propósito: si la misma persona alternara entre A y B, el test no mediría nada. **No cambiarlo a "sorteo por clic".**
- **Cómo verificar el reparto sin opinar (hecho el 2026-09-19):** N `curl` sin cookies leyendo `set-cookie: df_lp_roll` + el `landingVariant` del HTML → corte exacto en 500; y con `-H "Cookie: df_lp_roll=499|500"` → B|A. Para ver una variante puntual: `?lp=A` / `?lp=B` (OJO: esa visita se registra con esa variante). Datos reales: `landing_page_visits.landing_variant` por día, filtrando `is_bot`.
