...
/**
 * Reporte Quincenal de Marketing — corre domingos 06:00 AM Argentina (09:00 UTC),
 * pero solo en semanas ISO pares (cada 2 domingos). Ventana: últimos 14 días.
 *
 * Contenido: DOS tablas de embudo (una por origen: clase gratuita y tasación) (cantidades, % conversión, costo ARS/USD).
...
export default async function handler() {
  ...
  // Solo enviar en semanas ISO pares (cada 2 domingos)
  const now = new Date()
  const startOfYear = new Date(now.getFullYear(), 0, 1)
  const weekNumber = Math.ceil(((now.getTime() - startOfYear.getTime()) / 86400000 + startOfYear.getDay() + 1) / 7)
  if (weekNumber % 2 !== 0) {
    console.log(`[Biweekly Report] Semana impar ${weekNumber}, se omite`)
    return
  }
  ...
  try {
    ...
    console.error('[Biweekly Report] No se pudo crear el cliente Supabase:', err)
    return
  }
  ...
  try {
    const { data: settings } = await supabase
      ...
      .select('recipients, biweekly_enabled')
      .eq('id', 'default')
      .maybeSingle()
    const s = settings as { recipients?: string[]; biweekly_enabled?: boolean } | null
    const recipients = s?.recipients ?? []
    if (!s || s.biweekly_enabled !== true || recipients.length === 0) {
      const reason = !s ? 'no existe la fila report_settings (id=default)' : s.biweekly_enabled !== true ? 'reporte quincenal deshabilitado' : 'no hay destinatarios configurados'
      console.log(`[Biweekly Report] Omitido: ${reason}`)
      await _logRow(supabase, REPORT_TYPE, recipients, '(biweekly) omitido', 'skipped', reason)
      return
    }
    ...
    if (existing && existing.length > 0) {
      console.log('[Biweekly Report] Ya enviado hoy, se omite duplicado')
      await _logRow(supabase, REPORT_TYPE, recipients, '(biweekly) duplicado', 'skipped', 'ya enviado hoy')
      return
    }

    // Rango: últimos 14 días (de hace 13 hasta ayer).
    const toD = new Date(now); toD.setUTCDate(toD.getUTCDate() - 1)
    const fromD = new Date(toD); fromD.setUTCDate(fromD.getUTCDate() - 13)
    ...
    try {
      ...
      console.log('[Biweekly Report] Enviado OK')
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Unknown error'
      console.error('[Biweekly Report] Error al enviar:', msg)
      await _logRow(supabase, REPORT_TYPE, recipients, subject, 'failed', msg, { from, to, ...data })
    }
  } catch (err) {
    const msg = err instanceof Error ? err.message : 'Unknown error'
    console.error('[Biweekly Report] Error inesperado:', msg)
    await _logRow(supabase, REPORT_TYPE, [], '(biweekly) error', 'failed', msg)
  }
}

export const config: Config = {
  schedule: '0 9 * * 0', // Domingo 09:00 UTC = 06:00 AM Argentina; quincenal vía check de semana par
}
