...
<div style="max-width:680px;margin:0 auto;padding:32px 16px;">
  ...
  const subject = `Mensual Marketing (${monthName}) — ${totalLeads} leads | Gasto ${fmt(totalSpend)} | CPL ${avgCpl !== null ? fmt(avgCpl) : 'N/A'}`
  ...
  try {
    ...
    await supabase.from('email_report_log').insert({
      ...
      subject,
      status: 'sent',
      data_snapshot: { month: monthName, total_leads: totalLeads, total_spend: totalSpend, avg_ctr: avgCtr, avg_cpl: avgCpl },
    })
