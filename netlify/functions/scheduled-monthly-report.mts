...
/**
 * Reporte Mensual de Marketing — corre el primer sábado del mes 06:00 AM
 * Argentina (09:00 UTC). Ventana: mes calendario ANTERIOR completo.
 *
...
export default async function handler() {
  ...
  try {
    ...
    console.error('[Monthly Report] No se pudo crear el cliente Supabase:', err)
    return
  }
  ...
  try {
    const { data: settings } = await supabase
      ...
      .eq('id', 'default')
      .maybeSingle()
    const s = settings as { recipients?: string[]; monthly_enabled?: boolean } | null
    const recipients = s?.recipients ?? []
    if (!s || s.monthly_enabled !== true || recipients.length === 0) {
      const reason = !s ? 'no existe la fila report_settings (id=default)' : s.monthly_enabled !== true ? 'reporte mensual deshabilitado' : 'no hay destinatarios configurados'
      console.log(`[Monthly Report] Omitido: ${reason}`)
      await _logRow(supabase, REPORT_TYPE, recipients, '(monthly) omitido', 'skipped', reason)
      return
    }
    ...
    if (existing && existing.length > 0) {
      console.log('[Monthly Report] Ya enviado hoy, se omite duplicado')
      await _logRow(supabase, REPORT_TYPE, recipients, '(monthly) duplicado', 'skipped', 'ya enviado hoy')
      return
    }

    // Rango: mes calendario anterior completo (en UTC).
    ...
    try {
      ...
      console.log('[Monthly Report] Enviado OK')
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Unknown error'
      console.error('[Monthly Report] Error al enviar:', msg)
      await _logRow(supabase, REPORT_TYPE, recipients, subject, 'failed', msg, { from, to, ...data })
    }
  } catch (err) {
    const msg = err instanceof Error ? err.message : 'Unknown error'
    console.error('[Monthly Report] Error inesperado:', msg)
    await _logRow(supabase, REPORT_TYPE, [], '(monthly) error', 'failed', msg)
  }
}

// schedule REMOVIDO 2026-06-26: el cron real corre por pg_cron (job report-monthly).
