...
export default async function handler() {
  ...
  // Fase 6 — Embudo CRM (RPCs de Fase 3)
  let funnelSection = ''
  try {
    ...
    console.error('[Monthly Report] funnel CRM error:', err)
  ...
  try {
    ...
    if (pRes.ok) {
      ...
      for (const pipeline of pipelines) {
        ...
        console.log(`[Monthly Report] Commercial actions: ${JSON.stringify(commercialActions)}`)
      }
    } else {
...
<div style="max-width:680px;margin:0 auto;padding:32px 16px;">
  ...
  <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;padding:32px;">
    ...
    <h2 style="color:#1f2937;font-size:18px;margin:0 0 12px;">&#9733; Acciones Comerciales del Mes</h2>
    <div style="display:flex;gap:12px;margin-bottom:32px;flex-wrap:wrap;">
