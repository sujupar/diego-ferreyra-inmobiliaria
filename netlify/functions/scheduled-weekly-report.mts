...
export default async function handler() {
  ...
  if (existingReport && existingReport.length > 0) {
    console.log('[Weekly Report] Already sent today, skipping duplicate')
    return
  }
  ...
  try {
    ...
    if (pRes.ok) {
      ...
      if (pipelines.length === 0) {
        console.warn('[Weekly Report] Pipeline "🟢 GESTIÓN COMERCIAL - PROPIETARIOS" not found')
  ...
  // Fetch previous week pipeline for comparison
  ...
  try {
    ...
    console.error('[Weekly Report] Previous pipeline fetch error:', err)
  }
  ...
  try {
    ...
    console.log(`[Weekly Report] GHL calls: ${callStats.total} call conversations found (${pagesScanned + 1} pages scanned)`)
  } catch (err) {
  ...
  }).join('')}</tbody></table>${prevPipelineStages.length > 0 ? `<p style="color:#9ca3af;font-size:11px;margin:8px 0 0;">Comparado con la semana anterior</p>` : ''}` : (
...
export const config: Config = {
  schedule: '0 9 * * 6', // Saturday 09:00 UTC = 6:00 AM Argentina (UTC-3)
}
