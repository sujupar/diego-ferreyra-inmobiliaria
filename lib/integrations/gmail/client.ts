...
/**
 * Cliente Gmail API de solo lectura, vía cuenta de servicio con
 * **domain-wide delegation** (Google Workspace).
 *
 * No usa la librería googleapis ni google-auth-library — firma el JWT con el
 * `crypto` nativo de Node (RS256) y pega a la REST API directamente. Esto evita
 * sumar dependencias pesadas al bundle de las rutas/cron.
 *
 * Requisitos (env vars):
 ...
 *
 * El admin de Workspace debe autorizar el Client ID de la SA con el scope
 * `gmail.readonly` en Admin Console → Seguridad → Controles de API → Delegación.
 */
...
/** True si las 3 env vars necesarias están presentes. */
...
async function getAccessToken(): Promise<string> {
  if (!gmailConfigured()) {
    throw new GmailError('Gmail no configurado: faltan GMAIL_SA_CLIENT_EMAIL / GMAIL_SA_PRIVATE_KEY / GMAIL_IMPERSONATE_EMAIL')
  }
  ...
  const claims = base64url(
    JSON.stringify({
      ...
      sub: process.env.GMAIL_IMPERSONATE_EMAIL, // domain-wide delegation: impersonar la casilla
      scope: SCOPE,
      aud: TOKEN_URL,
      iat: now,
      exp: now + 3600,
